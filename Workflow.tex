...
\item Have found that the core of the manipulations of estimates lies in \textt{function.ssdd} and \textt{m.ssdd}, and \textt{function.detrend} and \textt{m.detrend}, for the "sum.sum" and "detrend" plot representations respectively. The \textt{ssdd} elements generate the sums of double differences for each point on each dimension (out of A, P, and C). The \textt{detrend} elements are performing some form of detrending and reallocation of anchor points that I don't yet fully understand.
...
\item AGENDA: clear up a few questions around \textt{ssdd} elements, and then need to focus on understanding \textt{detrend} elements.
