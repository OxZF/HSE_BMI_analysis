...
# revisions of write.functions.v2
...
apc.get.design.indiv.collinear <- function(data, unit=1, datatype, 
  ...
  if (!isTRUE(datatype %in% datatype.list))
    stop("datatype not recognised; must be one of 'AP' 'AC rectangular' 'CP' 'AC triangular' 'AP trapezoid'. ")
  ...
  {
    if (n.coh.excl.end==0 & n.coh.excl.start==0) stop("datatype is AP trapezoid but no cohorts excluded")
    L <- I-1  
    i.index <- 1:I
    j.index <- (L+1):(L+J)
    i.value.mu <- rep(i.index, each=J)
  ...
  if (anyNA(full.design.collinear)) stop("NA in design matrix, check datatype or n.coh.excl")
...
apc.get.design.indiv.model <- function(apc.get.design.indiv.collinear, model.design = "APC",
  ...
  #### check model.design ####
  design.list <- c("APC", "AP", "AC", "PC", "Ad", "Pd", "Cd", "A", "P", "C", "t", "tA", "tP", "tC", "1")
...
##### write apc.fit.indiv #####
apc.fit.indiv <- function (apc.get.design.indiv.model, model.family=NULL, n.coh.excl.start = 0, n.coh.excl.end = 0, DV=NULL){
